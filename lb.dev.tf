...
resource "aws_lb" "dev_lb" {
  name               = format("%s-dev-lb", var.project)
  internal           = false
  ...
  access_logs {
    ...
    prefix  = format("%s-dev-log-", var.project)
    enabled = true
  }
...
resource "aws_lb_target_group" "dev_servers" {
  name     = format("%s-dev-lb-tg", var.project)
  port     = 80
  protocol = "HTTP"
...
resource "aws_lb_listener" "dev_https" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.dev_servers.arn
  }
}

#Creating Launch Configuration
resource "aws_launch_configuration" "dev_lc" {
  name_prefix     = format("%s-dev-lc-", var.project)
...
resource "aws_autoscaling_group" "dev_asg" {
  name              = format("%s-dev-asg", var.project)
  min_size          = var.dev_min_instance_count
  max_size          = var.dev_max_instance_count
  health_check_grace_period = 300
  health_check_type = "ELB"
  target_group_arns = [aws_lb_target_group.dev_servers.arn]
  launch_configuration = aws_launch_configuration.dev_lc.id
  ...
  tag {
    key                 = "Name"
    value               = format("%s-dev-server", var.project)
...
resource "aws_autoscaling_policy" "dev_scale_up" {
  name                   = format("%s-dev-scale-up", var.project)
...
resource "aws_autoscaling_policy" "dev_scale_down" {
  name                   = format("%s-dev-scale-down", var.project)
  scaling_adjustment     = -1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 180
  autoscaling_group_name = aws_autoscaling_group.dev_asg.name
...
# Scaling dev_asg based on cloudwatch metrics
resource "aws_cloudwatch_metric_alarm" "dev_high_cpu" {
  alarm_name          = format("%s-dev-high-cpu", var.project)
  ...
  statistic           = "Average"
  threshold           = "70"
  alarm_description   = format("This metric monitors %s-dev high cpu utilization", var.project)
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.dev_asg.name
  }

  alarm_actions = [aws_autoscaling_policy.dev_scale_up.arn]
}

resource "aws_cloudwatch_metric_alarm" "dev_low_cpu" {
  alarm_name          = format("%s-dev-low-cpu", var.project)
  ...
  statistic           = "Average"
  threshold           = "40"
  alarm_description   = format("This metric monitors %s-dev low cpu utilization", var.project)
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.dev_asg.name
  }

  alarm_actions = [aws_autoscaling_policy.dev_scale_down.arn]
}